Attribute VB_Name = "Game_MD5"
' ######################################
' ##
' ##  LightQ MD5 calculation
' ##
...
Public Function MD5(strString As String) As Variant

    Dim bytBuffer()       As Byte
    Dim MD5Array(1 To 16) As Byte
    Dim c                 As Byte
    Dim i                 As Byte
    
    bytBuffer = MD5StringArray(strString)
    
    MD5Start
    MD564Split Len(strString), bytBuffer
    MD5Finish
    
    c = 1
    For i = 1 To Len(MD5Value) Step 2
       MD5Array(c) = Val("&h" & Mid(MD5Value, i, 2))
       c = c + 1
    Next i
